...
const ESP32_TEMPLATE = `#include <WiFi.h>
#include <WebServer.h>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { max-width: 600px; margin: 0 auto; background: white; padding: 15px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
        ...
        .section h3 { margin-bottom: 10px; color: #2c5282; font-size: 1.1em; }
        .field { display: flex; justify-content: space-between; align-items: center; padding: 5px 0; border-bottom: 1px solid #eee; }
        .field:last-child { border-bottom: none; }
        .label { font-weight: bold; color: #555; font-size: 0.9em; }
        .value { color: #333; font-size: 0.9em; }
        .status { padding: 3px 6px; border-radius: 3px; color: white; font-size: 0.8em; }
        .status.available { background: #48bb78; }
        .status.charging { background: #3182ce; }
        .status.offline { background: #e53e3e; }
        .checkbox { width: 16px; height: 16px; }
        .refresh-btn { background: #3182ce; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer; margin: 8px 0; font-size: 0.9em; }
...
async function buildESP32Code() {
  try {
    console.log('Минификация HTML...');
    
    const minifiedHtml = await minify(WEB_HTML, {
    ...
    console.log('Встраивание HTML в C++ код...');
    
    // Экранируем специальные символы для C++
    ...
    const finalCode = ESP32_TEMPLATE.replace('{{WEB_CONTENT}}', escapedHtml);
    
    // Сохраняем готовый код
    fs.writeFileSync('esp32/charging_station.ino', finalCode);
    
    console.log('✅ ESP32 код готов: esp32/charging_station.ino');
    console.log(`📦 Размер HTML: ${Math.round(minifiedHtml.length / 1024 * 100) / 100} KB`);
